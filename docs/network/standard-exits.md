...
You can exit only 1 UTXO at a time. Consider [merging multiple UTXOs](/network/utxos) into 1 UTXO if you don't want to exit all of your funds at once with a single UTXO. By default, the priority is given to UTXOs that were created the earliest.
...
async function startStandardExit() {
  ...
  console.log("Alice started a standard exit: " + receipt.transactionHash);
