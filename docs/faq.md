...
## Why does a smaller UTXO set on the Plasma Chain reinforce the safety of user funds in a mass exit event? 

Consider a scenario whereby a dishonest Plasma Chain operator steals user funds by creating an invalid block, and then initiates a `Standard Exit` on his UTXOs. In such a scenario, users must exit their UTXOs back to the root chain in order to safely retain ownership of their funds. 
...
A smaller UTXO set on the Plasma Chain can mitigate these vulnerabilities, thereby reinforcing the safety of user funds. For this reason, users are encouraged to merge their UTXOs continuously.
