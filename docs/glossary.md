...
## API
Application Programming Interface. Acts as the interface between different software programs, for example, between OmiseGO eWallet and a provider's existing wallet platform.
...
## eWallet Server
The server component of the eWallet Suite. 


## eWallet Suite
OmiseGO's free and open source software toolkit, including client and server applications, and SDKs for building a complete eWallet solution. The eWallet Suite includes the eWallet Server, the Admin Panel, the AdminAPI and ClientAPI, and Point-of-Sale app examples. eWallet suite is built to seamlessly onboard merchants and users to the OmiseGO Network. It allows users to make and accept transactions with fiat, crypto, and or any type of digitally-represented currency or asset. 
...
Integration libraries exist in the layer between web applications and the blockchain, providing the end-to-end infrastructure for integrating an end-user application, from application to library to watcher to child chain. The eWallet, as a web app, also uses this infrastructure to connect to the blockchain.
...
## Users
In eWallet suite, a user can either be a regular user or an admin user. Regular users are those using the applications created by providers (mobile applications for example), and who were added to the eWallet database by the provider. An admin user is able to access the OmiseGO admin panel.
...
## Wallet
A wallet holds value in a balance, identified by an address that is used to group transactions. A user can have one or more balances, and a balance can contain different currencies. You can learn more about wallets at https://github.com/omisego/ewallet/blob/master/docs/design/wallets.md
