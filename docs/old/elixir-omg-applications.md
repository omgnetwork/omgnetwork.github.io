...
See [application architecture](https://github.com/omisego/elixir-omg/blob/master/docs/architecture.md) for more details.
...
**Note** *If you wish to configure the PORT that child chain is using without recompiling the code, you would need to use the environment variable *PORT*. *Post-compile (post release), you'll need to edit the following file: sys.config
...
See the [API Documentation](https://omisego.github.io/elixir-omg)
...
For more on the responsibilities and design of the Watcher see [Tesuji Plasma Blockchain Design document](docs/blockchain_design.md).
...
See [exit validation documentation](https://github.com/omisego/elixir-omg/blob/master/docs/exit_validation.md) for details.
