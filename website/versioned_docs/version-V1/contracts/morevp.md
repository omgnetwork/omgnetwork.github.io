...
A key component of the Plasma MVP design is a protocol for exits where a user may withdraw back to the root chain any funds available to them on the Plasma chain.
...
* Describes the attack vectors that exist in the protocol, how these vectors can be mitigated and isolated to a relatively small attack surface.
...
Future development work will focus on decreasing the implementation complexity of the design and minimizing contract gas usage.
...
**The owner of output out to a transaction tx must prove that:**
...
The MoreVP mechanism is designed to be deployed to Ethereum and, as a result, some particulars of this specification take into account the limitations of the EVM.
...
However, we avoided describing how users prove the statements they’re required to prove.
...
The MoreVP protocol guarantees that if a transaction is exitable, then either the unspent inputs or unspent outputs can be withdrawn, depending on whether the transaction is canonical.
However, in a scenario where MoreVP exits are required, users may not be aware that an in-flight transaction is non-canonical. This can occur when the owner of an input to an in-flight transaction is malicious and has signed a second transaction spending the same input.
...
The MoreVP exit protocol uses a challenge-response mechanism. It allows users to submit a challenge subject to a response that may invalidate the challenge. The exit process is split into two periods, which gives users enough time to respond to a challenge.

**When challenges require a response ...**

* Challenges must be submitted before the end of the first exit period.
...
Any user may initiate an exit following such steps:

1. Present a spend transaction.
2. Prove that the transaction is exitable.
3. Submit an exit bond to start the action. Later, the exit bond is used to cover the cost for other users to publish statements about the canonicity of the transaction in question.
...
> Other methods exist to prove a transaction is exitable. And there may be further ways to optimize the two methods described here. Further research is required to provide a deterministic ordering of exits by some priority. MoreVP exits are assigned a priority based on the position in the Plasma chain of the most recently included (newest) input to that transaction. Unlike the MVP protocol, each input and output to a transaction is assigned the same priority.
...
Proving a transaction canonical is expensive. Thus, by default, we assume the referenced transaction is canonical, and it allows a series of challenges and responses to determine the true canonicity of the transaction.
...
Some participants in a transaction may not be aware that the transaction is non-canonical. Owners of both inputs and outputs to a transaction may want to start an exit to receive the funds from the exit.
...
1. Input or output owner places a 'piggyback' bond that covers the cost of challenges allowing to show the input or output is spent.
...
1. Implemented solution:
    ...
    * Contract verifies if other types of exits exist for particular output when a standard exit is submitted. In this case, the new exit is blocked.

    * When an in-flight exit is added, the contract verifies whether the standard exit is either in progress or finalized. In this case, the in-flight exit is marked as one that can be exited only from inputs, and problematic inputs are marked as spent for piggybacking purposes.
...
2. `TX1` is in-flight.
3. The operator begins withholding blocks while `TX1` is still in-flight.
   Neither Alice nor Bob knows if the transaction has been included in a block.
4. Someone with access to `TX1` (Alice, Bob, or otherwise) starts an exit referencing `TX1` and places `exit bond`.
5. Bob piggybacks onto the exit and places a `piggyback bond`.
...
2. `TX1` is in-flight.
3. The operator begins withholding blocks while `TX1` is still in-flight.
   Neither Mallory nor Bob knows whether the transaction has been included in a block.
...
7. Bob piggybacks onto the exit and places a `piggyback bond`.
8. In period 1, someone challenges the canonicity of `TX1` by revealing `TX2`.
9. No one can respond to the challenge in period 2, so `TX1` is determined to be non-canonical.
...
4. The operator spends `UTXO3` in `TX3`, creating `UTXO4`.
5. The operator starts an exit referencing `TX3` and places `exit bond`.
6. Operator piggybacks onto the exit and places `piggyback bond`.
7. Bob starts a *standard* exit for `UTXO2`.
8. The operator’s exit will have priority of position of `UTXO3`.
...
8. Bob piggybacks onto the exit and places a `piggyback bond`.
9. Alice is honest, so she hasn’t spent `UTXO1` in any transaction other than `TX1`.
10. The operator’s exit will have priority of position of `UTXO3`.
...
8. Bob piggybacks onto the exit and places a `piggyback bond`.
9. Alice piggybacks onto the exit and places a `piggyback bond`.
...
10. The operator's exit of `TX3` will have priority of position of `UTXO3`.
...
The out-of-band process in such an event should always put the burden of starting in-flight exits **on the sender**.
...
7. No one can respond to the challenge in period 2, so `TX1` is determined to be non-canonical.
...
An honest user can start an exit and have their exit bond slashed.
...
4. Alice piggybacks onto the exit and places a `piggyback bond`.
...
> See section [Timeouts](#Timeouts) for details about one more possible mitigation. However, due to the uncertainty of timeouts in MoreVP, other mitigations for the retry problem might be necessary.
...
We can reduce the impact of this attack by minimizing the gas cost of exiting and the value of an `exit bond`.
...
However, we believe that challenges can be sufficiently incentivized if the `exit bond` simply covers the gas cost of challenging.
...
Our system requires only a single node to be properly incentivized to challenge, and it’s likely that many node operators will have strong external incentives.
...
- reduce the number of transactions vulnerable to [**Honest Exit Bond Slashing**](#Honest-Exit-Bond-Slashing) point in time.
- alleviate [**Honest transaction retries attack**](#Honest-transaction-retries-attack), allowing Alice to just wait for the timeout and retry
...
The set of canonical transactions is any transaction that is the first of all its competitors.
